...
export default function TopsPage() {
  const col = useAdminStore((s) => s.getCollection("tops"));
  ...
  return (
    <main className="bg-white text-zinc-900 min-h-screen overflow-x-hidden">
      ...
      <section className="relative w-full h-[65vh] overflow-hidden">
        <Image
          src={col?.featured ?? "/collections/men_shirt/shirtCollection.jpeg"}
          alt={col?.title ?? "The Anti-Uniform"}
          fill
          priority
        ...
        <div className="absolute bottom-0 left-0 flex flex-col gap-5 p-10 md:p-16 lg:p-20 max-w-2xl">
          ...
          >
            {col?.tagline ?? "Move in silence. Let the fabric do the talking."}
          </motion.p>
        </div>
      </section>
      ...
      <section className="max-w-360 mx-auto px-6 md:px-12 lg:px-16 pt-10 pb-32">
        ...
        >
          {products.map((product) => (
            ...
            >
              <Link href={`/collections/tops/${product.id}`} className="block">
